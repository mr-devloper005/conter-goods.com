...
function SearchResultCard({ post, index }: { post: SitePost; index: number }) {
  ...
  return (
    <Link href={href} className={`group block overflow-hidden rounded-[10px] border border-[var(--editable-border)] bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-2xl ${strong ? 'md:col-span-2' : ''}`}>
      {image ? (
      ...
      <div className="p-5 sm:p-6">
        ...
        <h2 className="editable-display mt-4 line-clamp-3 text-2xl font-semibold leading-tight text-[#171423]">{post.title}</h2>
...
export default async function SearchPage({ searchParams }: { searchParams?: Promise<{ q?: string; category?: string; task?: string; master?: string }> }) {
  ...
  return (
    <EditableSiteShell>
      <main className="min-h-screen bg-[var(--slot4-warm)] text-[#171423]">
        <section className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
          <div className="grid gap-8 rounded-[10px] border border-[var(--editable-border)] bg-white p-6 shadow-[0_30px_90px_rgba(15,23,42,0.08)] backdrop-blur md:grid-cols-[0.8fr_1.2fr] lg:p-10">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.28em] text-[var(--slot4-accent)]">{pagesContent.search.hero.badge}</p>
              <h1 className="editable-display mt-5 text-5xl font-semibold leading-[1.02] text-[#171423] sm:text-6xl">{pagesContent.search.hero.title}</h1>
              <p className="mt-6 max-w-xl text-base font-semibold leading-8 opacity-70">{pagesContent.search.hero.description}</p>
            </div>
            <form action="/search" className="self-end rounded-[10px] border border-[var(--editable-border)] bg-[var(--slot4-warm)] p-4 sm:p-5">
              ...
              <button className="mt-3 inline-flex h-12 w-full items-center justify-center rounded-2xl bg-[var(--slot4-accent)] px-6 text-sm font-bold uppercase tracking-[0.18em] text-white transition hover:-translate-y-0.5" type="submit">Search</button>
            </form>
          </div>
          ...
          <div className="mt-10 flex flex-wrap items-end justify-between gap-4">
            <div>
              ...
              <h2 className="editable-display mt-2 text-3xl font-semibold">{query ? `Results for "${query}"` : pagesContent.search.resultsTitle}</h2>
            </div>
          ...
          {results.length ? (
            ...
            <div className="mt-8 rounded-[10px] border border-dashed border-[var(--editable-border)] bg-white p-10 text-center">
