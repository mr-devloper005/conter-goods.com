...
export default function CreatePage() {
  ...
  const task = useMemo(() => (SITE_CONFIG.tasks.find((item) => item.enabled)?.key || 'article') as TaskKey, [])
  ...
  return (
    <EditableSiteShell>
      <main className="min-h-screen bg-[var(--editable-page-bg,#fff7ee)] text-[var(--editable-page-text,#2f1d16)]">
        <section className="mx-auto max-w-3xl px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
          <div className="text-center">
            ...
            <p className="mx-auto mt-5 max-w-xl text-base font-semibold leading-8 opacity-70">{pagesContent.create.hero.description}</p>
          </div>

          <form onSubmit={submit} className="mt-10 rounded-[2.2rem] border border-[var(--editable-border)] bg-white/75 p-5 shadow-[0_30px_90px_rgba(15,23,42,0.08)] backdrop-blur sm:p-8">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <h2 className="text-3xl font-black tracking-[-0.06em]">{pagesContent.create.formTitle}</h2>
              <span className="rounded-full bg-[var(--editable-page-bg,#fff7ee)] px-4 py-2 text-xs font-black uppercase tracking-[0.16em]">{session.name}</span>
            </div>
